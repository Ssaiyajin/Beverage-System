...
public class DB {
    ...
    private List<Crate> initCrates() {
        // use indices defensively; bottles list is initialized before crates
        return new ArrayList<>(List.of(
    ...
    private List<Order> initOrders() {
        return new ArrayList<>(List.of(
                ...
                new Order(2, new ArrayList<>(List.of(
                ...
                new Order(3, new ArrayList<>(List.of(
    ...
    /**
     * Read-only accessors: return unmodifiable views to prevent external mutation.
     */
    ...
    /**
     * Preferred crate getter. kept geCrate for backward compatibility.
     */
    ...
    @Deprecated
    public Crate geCrate(final int id) {
        return getCrate(id);
    }

    public List<Beverage> getBeverages() {
        return Stream.concat(this.bottles.stream().map(b -> (Beverage) b),
                             this.crates.stream().map(c -> (Beverage) c))
                .collect(Collectors.toUnmodifiableList());
    }

    public List<Beverage> getBeverages(double min, double max) {
        final List<Beverage> filteredBottles = this.bottles.stream()
                .filter(b -> b.getPrice() >= min && b.getPrice() <= max)
                .map(b -> (Beverage) b)
                .collect(Collectors.toList());

        final List<Beverage> filteredCrates = this.crates.stream()
                .filter(c -> c.getPrice() >= min && c.getPrice() <= max)
                .map(c -> (Beverage) c)
                .collect(Collectors.toList());

        return Stream.concat(filteredBottles.stream(), filteredCrates.stream())
                .collect(Collectors.toUnmodifiableList());
    }

    public Order addOrder(final Order newOrder) {
        Objects.requireNonNull(newOrder, "newOrder must not be null");
        int nextId = this.orders.stream().map(Order::getOrderId).max(Comparator.naturalOrder()).orElse(0) + 1;
    ...
    public void updateBottle(Bottle bottle, boolean isStockIncrease) {
        ...
        this.bottles.stream()
                ...
                .findFirst()
                .ifPresent(b -> b.setInStock(isStockIncrease ? b.getInStock() + bottle.getInStock()
    ...
    public void updateCrate(Crate crate, boolean isStockIncrease) {
        ...
        this.crates.stream()
                ...
                .findFirst()
                .ifPresent(c -> c.setInStock(isStockIncrease ? c.getInStock() + crate.getInStock()
                        : c.getInStock() - crate.getInStock()));
    }

    public Order getOrder(int orderId) {
        return this.orders.stream().filter(c -> c.getOrderId() == orderId).findFirst().orElse(null);
    }

    public void changeOrderStatus(Order order, OrderStatus status) {
        if (order == null || status == null) return;
        this.orders.stream()
                .filter(c -> c.getOrderId() == order.getOrderId())
                .findFirst()
    ...
    public Beverage createOrUpdateBeverage(Beverage beverage) {
        ...
        if (beverage instanceof Bottle bottle) {
            return updateBottleDb(bottle);
        } else if (beverage instanceof Crate crate) {
            return updateCrateDb(crate);
        }
        return null;
    }
